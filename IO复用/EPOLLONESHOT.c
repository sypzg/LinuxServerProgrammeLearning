 ...
 void addfd( int epollfd, int fd, bool onseshot )
 {
     epoll_event event;
     event.data.fd = fd;
     event.events = EPOLLIN | EPOLLET;
     event.events |= onsehot ? EPOLLONESHOT : 0;
...
int main( int argc, char* argv[] )
{
    ...
    assert(epollfd != -1 );
    /*注意，监听socket listenfd上是不能注册EPOLLONSHOT事件的，否则应用程序只能处理一个客户连接!因为后续的客户连接请求将不再触发listenfd上
    的EPOLLIN事件
    */
